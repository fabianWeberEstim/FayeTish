...
export const postFetishAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime): Promise<boolean> => {
        try {
            ...
            const pendingRequests = requests.filter(req => !req.postId && req.isValid);
    ...
    handler: async (runtime: IAgentRuntime): Promise<void> => {
        try {
            ...
            const requests = await runtime.cacheManager.get<FetishRequest[]>("valid_fetish_requests") || [];
            const nextRequest = requests.find(req => !req.postId && req.isValid);
            ...
            const tweet = await runtimeWithTwitter.twitterClient.tweet(tweetText);

            // بروزرسانی درخواست با شناسه پست
            nextRequest.postId = tweet.id;
            await runtime.cacheManager.set("valid_fetish_requests", requests);

            elizaLogger.debug(`Posted fetish request ${nextRequest.id} with tweet ID: ${tweet.id}`);
        } catch (error) {
